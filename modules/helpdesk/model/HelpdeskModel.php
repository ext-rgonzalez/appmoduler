...
class ModeloHelpdesk extends DBAbstractModel {
    private $_modulo=5;
    protected $cod;
...
//             la carga al servidor y optimizar el proceso.
    public function get_form($metodo,$usuario,$form,$data=0,$ciclo=1,$codSubM=null,$cMet=""){
        ...
        ModeloHelpdesk::get_boton($metodo,$usuario,$form, $cMet);
    }
    #traemos la tabla
...
//descripcion: Metodo para traer datos de una consulta dependiendo de los parametros de entrada
    public function get_datos($clave=null, $condicion="", $table='dual', $aux = null, $aux_1=null) {
        #traemos los datos segun la referencia de la entrada de la funcion
        $this->rows = "";
        $this->query = "";
        $this->query = $clave!="" ? "SELECT $clave FROM $table " : $aux_1;
...
//descripcion: metodo para ejecutar sentencias simples sin utilizar el modelo
    ...
    public function setRegistro($user_data = array()) {
        ...
        $valExt = '';
        $valName = '';
        if (isset($user_data['cod_' . str_replace('hd_', '', $user_data['no_esq_tabla'])]) and empty($user_data['cod_' . str_replace('hd_', '', $user_data['no_esq_tabla'])])) {
            $user_data['cod_' . str_replace('hd_', '', $user_data['no_esq_tabla'])] = ModeloHelpdesk::setSigSecuencia($user_data['no_esq_tabla']);
        }
        ...
        for($f=0;$f<$icount;$f++):
            ...
            if (isset($user_data["no_tmp_img".$t]) Or !empty($user_data["no_tmp_img".$t])):
                $err_img = $err_img . uploadImg($user_data, HD_DIR_ADJ, $valName, $colName,$t);
            endif;
        ...
        switch ($user_data['no_nom_tabla']) {
            case 'nuevaConfigHelpDesk':
                if ($user_data['cod_estado'] == 'AAA') {
                    $this->query = " call pbActualizaConfig(1," . $user_data['cod_config'] . ")";
                    $this->execute_single_query();
    ...
    public function setSigSecuencia($nomTbl) {
        $this->query = "SELECT IF(MAX(cod_" . str_replace('hd_', '', $nomTbl) . " IS NOT NULL),MAX(cod_" . str_replace('hd_', '', $nomTbl) . " + 1),1) as codSec 
                       FROM " . $nomTbl . " ";
        //print $this->query;exit;
        $this->get_results_from_query();
        if (count($this->rows) >= 1) {
