...
class ModeloDatolee extends DBAbstractModel {
    private $_modulo=11;
    protected $cod;
    public $_secuencia;
...
//             la carga al servidor y optimizar el proceso.
    public function get_form($metodo,$usuario,$form,$data=0,$ciclo=1,$codSubM=null,$cMet=""){
        ...
        ModeloDatolee::get_boton($metodo,$usuario,$form,$cMet);
    }
    #traemos la tabla
...
//descripcion: metodo para ejecutar sentencias simples sin utilizar el modelo
    ...
    public function setRegistro($user_data = array()) {
        ...
        if (isset($user_data['cod_' . str_replace('sys_','',str_replace('datolee_', '', str_replace('fa_', '',$user_data['no_esq_tabla'])))]) and empty($user_data['cod_' . str_replace('sys_','',str_replace('datolee_', '', str_replace('fa_', '',$user_data['no_esq_tabla'])))])) {
            $user_data['cod_' . str_replace('sys_','',str_replace('datolee_', '', str_replace('fa_', '',$user_data['no_esq_tabla'])))] = ModeloDatolee::setSigSecuencia($user_data['no_esq_tabla']);
        }
        ...
        for($f=0;$f<$icount;$f++):
            ...
            if (isset($user_data["no_tmp_img".$t]) Or !empty($user_data["no_tmp_img".$t])):
                $err_img = $err_img . uploadImg($user_data, DL_DIR_ADJ, $valName, $colName,$t);
            endif;
        ...
        switch ($user_data['no_nom_tabla']) {
            case 'NuevaVotos':
                $secuencia = ModeloDatolee::setSigSecuencia('datolee_bandeja_voto');
                $this->query = " INSERT INTO datolee_bandeja_voto
                                      ...
                                      VALUES (".$secuencia.",".$user_data["no_cod_usuario"] .",".$user_data["cod_voto"].",now(),'" . $user_data['cod_usuario'] . "','AAA')";
                $this->execute_single_query();
            break;   
        }
        ...
        switch ($user_data['no_nom_tabla']) {
            case 'NuevaVotos':
                if(isset($user_data["no_ind_reasignar"]) And $user_data["no_cod_usuario"]<>''):
                    ...
                    $this->query="UPDATE datolee_bandeja_voto SET cod_estado='BBB' WHERE cod_voto=".$user_data["cod_voto"];
                    $this->execute_single_query();
                    // registro la gestion automatica del sistema
                    $this->query="";
                    $secuencia_gestion = ModeloDatolee::setSigSecuencia('datolee_gestion');
                    $this->execute_single_query();
                    $this->query = " INSERT INTO datolee_gestion 
                                             (cod_gestion,obs_gestion,fec_gestion,fec_prox_voto,cod_voto,cod_usuario,cod_estado,cod_usuario_asignado)  
                                      VALUES (".$secuencia_gestion.",concat('Contacto reasignado ',now()),now(),'".$user_data["fec_prox_voto"]."'," .$user_data["cod_voto"]. "," .$user_data['cod_usuario']. ",'".$user_data["cod_estado"]."'," .$user_data['no_cod_usuario']. ")";
                    $this->execute_single_query();
